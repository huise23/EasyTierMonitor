# Tailscale Monitor Plugin

TrafficMonitor 插件，用于监控 Tailscale peer 的连接状态。

## 功能特性

- ✅ 实时监控 Tailscale peer 连接状态
- ✅ 显示直连/中转/断开状态
- ✅ 显示 Ping 延迟和 CLI 延迟
- ✅ 支持自定义显示（图标 + 文本）
- ✅ 详细信息悬浮提示
...
2. **安装插件**
   ```
   将 TailscaleMonitor.dll 复制到 TrafficMonitor 的 plugins 目录：
...
- 🔴 **断开**：无法连接到 peer
- ⚪ **未知/未找到**：Tailscale 未运行或选择的 peer 不在列表中

### 配置

配置文件位置：
```
TrafficMonitor 配置目录\tailscale_monitor_config.ini
```
...
├── res/                  # 资源文件
├── docs/                 # 文档
├── TailscaleMonitor.sln   # Visual Studio 解决方案
└── README.md             # 本文件
```
...
1. 确认 Tailscale 正在运行
2. 检查 tailscale 是否在 PATH 中
3. 尝试在命令行运行 `tailscale peer` 验证
...
- [Tailscale](https://github.com/Tailscale/Tailscale) - 简单易用的 VPN 解决方案
