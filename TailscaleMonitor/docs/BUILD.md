...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "输出位置: bin\x64\Release\TailscaleMonitor.dll"
} else {
...
```cmd
msbuild TailscaleMonitor.sln /p:Configuration=Release /p:Platform=x64 /t:Rebuild
```
...
1. **DLL 文件存在**
   ```cmd
   dir bin\x64\Release\TailscaleMonitor.dll
   ```
