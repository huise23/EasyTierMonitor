...
**文件位置**: `bin\x64\Release\TailscaleMonitor.dll`  
**大小**: 约 122 KB  
**状态**: ✅ 编译成功
...
2. **复制新DLL到插件目录**
   ```
   源文件: C:\work\ping1\TailscaleMonitor\bin\x64\Release\TailscaleMonitor.dll
...
**优先顺序：**
1. `C:\Users\<用户名>\AppData\Roaming\TrafficMonitor\tailscale_monitor_debug.log`
...
1. **DLL未加载**
   - 没有生成 `tailscale_monitor_debug.log`
   - 原因：DLL路径错误或位数不匹配
...
```powershell
# 搜索所有可能的日志位置
Get-ChildItem -Path C:\ -Filter "tailscale_monitor_debug.log" -Recurse -ErrorAction SilentlyContinue
```

### 问题：日志显示"找不到tailscale"

**解决**：
1. 打开配置对话框
2. 设置正确的CLI路径
3. 或确保tailscale在系统PATH中

---

## 📊 报告问题时请提供

1. **完整的调试日志内容** (`tailscale_monitor_debug.log`)
2. **错误日志** (`error.log`)
3. **DLL文件信息**（大小、修改时间）
4. **TrafficMonitor版本**
5. **Windows版本**
