...
```powershell
# 在DLL所在目录执行
dumpbin /HEADERS TailscaleMonitor.dll | Select-String "machine"
...
**或者**使用以下命令检查导出函数：
```powershell
dumpbin /EXPORTS TailscaleMonitor.dll | Select-String "TMPluginGetInstance"
```
...
dumpbin /DEPENDENTS TailscaleMonitor.dll
...
# 路径：Windows日志 → 应用程序
# 查找TrafficMonitor或TailscaleMonitor相关的错误
```
...
2. **复制新DLL**
   ```powershell
   Copy-Item "C:\work\ping1\TailscaleMonitor\bin\x64\Release\TailscaleMonitor.dll" "C:\Program Files\TrafficMonitor\plugins\" -Force
   ```
3. **启动TrafficMonitor**
4. **查看新的调试日志**
...
[timestamp] GetInfo(0) - [INFO] GetInfo(0) returning: 'Tailscale Monitor' (len=15)
...
1. **dumpbin输出**：
   ```powershell
   dumpbin /EXPORTS TailscaleMonitor.dll > exports.txt
   cat exports.txt
   ```
...
- `tailscale_monitor_debug.log` 的完整内容
- 如果仍然只有4条日志，提供上述诊断信息
